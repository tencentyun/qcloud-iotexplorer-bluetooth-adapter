...
export class CRC32 {
  ...
  updateValue(buffer, offset, len): void {
    for (let i = 0; i < len; i++) {
      const data = buffer[offset + i] ^ this.value;
      this.value = (this.crcTable[data & 0xFF] ^ this.value >>> 8) & 0xFFFFFFFF;
    }
  }
  ...
  update(buffer): void {
    if (buffer instanceof Int8Array) {
      this.updateValue(buffer, 0, buffer.length);
    } else if (typeof buffer === 'number') {
      this.update(new Int8Array([byteUtil.convertNumberToByte(buffer)]));
    }
  }
